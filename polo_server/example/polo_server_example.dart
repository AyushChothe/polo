...
class UserType implements PoloType {
  ...
  factory UserType.fromMap(Map<String, dynamic> map) {
    return UserType(name: map['name'] as String, age: map['age'] as int);
  }
  final String name;
  final int age;
...
void main() async {
  ...
  final server = await Polo.createServer();

  // Direct Server
  // PoloServer server =
  // await Polo.createServer(address: "127.0.0.1", port: 3000);

  // Register a Type
  server.registerType<UserType>(
    PoloTypeAdapter<UserType>(
      toMap: (type) => type.toMap(),
      fromMap: UserType.fromMap,
    ),
  );
